...
RUN dnf install -y openssh-server sudo pwgen hostname && ssh-keygen -A && useradd -m -s /bin/bash fedora && echo 'fedora ALL=(ALL) NOPASSWD:ALL' >/etc/sudoers.d/10-fedora-sudo
